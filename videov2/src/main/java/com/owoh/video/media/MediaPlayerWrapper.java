...
public class MediaPlayerWrapper implements MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener {
    ...
    public void setDataSource(List<String> dataSource) {
        ...
        for (int i = 0; i < dataSource.size(); i++) {
            ...
            info.setPath(path);
            info.setRotation(Integer.parseInt(rotation));
            info.setWidth(Integer.parseInt(width));
            info.setHeight(Integer.parseInt(height));
    ...
    public int getCurVideoDuration(){
        return mInfoList.get(curIndex).getDuration();
    }
    ...
    public int getVideoDuration() {
        ...
        int duration = 0;
        for (int i = 0; i < mSrcList.size(); i++) {
            duration += mInfoList.get(i).getDuration();
        }
        return duration;
    }
    ...
    public int getCurPosition() {
        ...
        for (int i = 0; i < curIndex; i++) {
            position += mInfoList.get(i).getDuration();
        }
    ...
    public void seekTo(int time) {
        ...
        for (int i = 0; i < mInfoList.size(); i++) {
            duration += mInfoList.get(i).getDuration();
            if (duration > time) {
                int ti = time - (duration - mInfoList.get(i).getDuration());
                if (curIndex == i) {
