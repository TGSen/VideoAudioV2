...
public class SurfaceDecoder {
    ...
    public void initDecodeVideoInfo(VideoInfo info) {
        try {
            ...
            mVideoInfo.setWidth(mediaFormat.getInteger(MediaFormat.KEY_WIDTH));
            mVideoInfo.setHeight(mediaFormat.getInteger(MediaFormat.KEY_HEIGHT));
    ...
    private int selectTrack(MediaExtractor extractor) {
        ...
        for (int i = 0; i < numTracks; i++) {
            ...
            if (mime.startsWith("com/owoh/video/")) {
                if (VERBOSE) {
