...
public class ColorPickerView extends SeekBar {
    ...
    // 默认状态下长边与短边的比例为 6 ：1
    private static final int defaultSizeShort = 50; // * 6
    private static final int defaultSizeLong = 400;
    ...
    public ColorPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mProgressColor = array.getColor(R.styleable.ColorPickerView_unGradualColor, 0);
...
//    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (curX == curY || curY == Integer.MAX_VALUE) {
            curX = mLeft + mRadius * 2;// getWidth() / 2;
            curY = getHeight() / 2;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
            ...
            if (progressListener != null) {
                progressListener.onPersent((float) curX / (float) (mRight - mLeft));
            }
